...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  node_t *curr;
  node_t *prev;
  node_t *ascend = node_alloc(newblk);
   /*
   ...
   */
   if (l->head == NULL) {
     ...
     if (curr->next == NULL) {
       ...
       if (ascend->blk->start <= curr->blk->start) {
         ascend->next = l->head;
         l->head = ascend;
       }
       else {
         while (curr != NULL && ascend->blk->start >= curr->blk->start) {
           prev = curr;
           curr = curr->next;
         }
         prev->next = ascend;
...
void list_coalese_nodes(list_t *l){ 
  ...
  while (curr) {
    if (prev->blk->end = curr->blk->start) {
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  node_t *curr = l->head;
  while(curr != NULL){ // Iterate through the list to find a node with a blk that has blk->pid = pid
    if(comparePid(pid, curr->blk)){ // USE the comparePID()
      return true;
     }
     curr = curr->next;
   }
