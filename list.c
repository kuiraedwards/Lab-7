  ...
  if(l->head == NULL;) {
    ...
    if(current->next == NULL) {
      if(newblk->start <= current->blk->start) {
        ...
        current->next = ascendNode;
        ascendNode->next = NULL;
      }
    }
    else{
      if(ascendNode->blk->start <= current->blk->start) {
        ascendNode->next = l->head;
        l->head = ascendNode;
      }
      else{
        while(current != NULL && ascendNode->blk->start >= current->blk->start) {
          previous = current;
          current = current->next;
        }
        previous->next = ascendNode;
        ascendNode->next = current;
      }
    }
  }
}
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  if(l->head == NULL) {
    ...
    prev = current = l->head;
    currblk_size = current->blk->end - current->blk->start +1;
    ...
    if(current->next == NULL) {
      ...
      if(newblk_size >= currblk_size) {
        ...
        while(current != NULL && newblk_size <= currblk_size) {
          prev = current;
          current = current->next;
          if(current != NULL) {
            currblk_size = current->blk->end - current->blk->start;
          }
          prev->next = newNode;
...
void list_add_descending_by_blocksize(list_t *l, block_t *blk){
  ...
  if(l->head == NULL){
    ...
    if(current->next == NULL) {  //only one node in list
       if(newblk_size >= currblk_size) {  // place in front of current node
          newNode->next = l->head;
          l->head = newNode;
       }
